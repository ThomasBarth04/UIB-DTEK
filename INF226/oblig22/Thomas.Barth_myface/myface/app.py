...
class User(UserMixin, Box):
    ...
    def save(self):
        # prepare info JSON
        info = json.dumps({k: self[k] for k in self if k not in [
...
def create_app(test_config=None):
    ...
    def login_callback():
        try:
            ...
            if not userinfo:
                flask.flash(
                    "Failed to retrieve user info from GitLab.", category="error")
            ...
            flask.flash("GitLab login failed. Please try again.",
                        category="error")
            return redirect(url_for("login"))
