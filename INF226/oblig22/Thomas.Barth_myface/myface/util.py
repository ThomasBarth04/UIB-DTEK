...
def load_or_generate_secrets(app: Flask):
    ...
    secrets_path = os.path.join(app.instance_path, f"secrets{
                                '_debug' if app.debug else ''}.cfg")
    tmpl_path = os.path.join(app.root_path + "/" +
                             app.template_folder + "/" + "_secrets.tmpl")
    ...
    if not os.path.exists(secrets_path) and os.path.exists(tmpl_path):
        ...
        if generated_secret_key:
            ...
            with open(secrets_path, "x") as outfile:
                outfile.write(data.replace("SECRET_KEY=None",
                              f"SECRET_KEY='{generated_secret_key}'"))
...
class LogFilter(logging.Filter):
    def filter(self, record):
        try:
            ...
            record.sep = " "
            if request:
                record.remote_addr = f"{
                    request.remote_addr or '(unknown)':15s} "
...
def setup_logging(app: Flask):
    ...
    try:
        ...
        _streamHandler.setFormatter(
            coloredlogs.ColoredFormatter(format, style="{"))
    except:
